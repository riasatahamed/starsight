...
let panX = 0, panY = 0;
// Mobile pan fast-path: render interaction frames at 60 FPS while temporarily
// skipping only the expensive Milky Way/photo layer. Astronomical objects keep
// their normal quality and cached calculations.
let mobilePanActive = false;
let panVelocityX = 0, panVelocityY = 0;
let lastPanMoveTime = 0;
let mobilePanLastFrame = 0;
let mobilePanInertia = false;
let mobilePanFriction = 0.88;
let mobilePanStopThreshold = 0.08;
let rotateOffset = 0;             
...
/* =========================================================
   ...
   Keeps astronomical math accurate while separating it from
   visual frame rate. Mobile devices render at an adaptive rate,
   use a lower canvas DPR, cull faint stars and cache slow math.
   ========================================================= */
const PERF = {
    mobile: false,
    lowEnd: false,
    ...
    adaptiveCooldown: 0,
    starCacheStamp: 0,
    starCacheLocationKey: '',
    paused: false,
    mobileStars: null,
    sortedStars: null,
    starByName: new Map(),
    objectCacheStamp: 0,
    objectCacheLocationKey: '',
    cachedPlanets: null,
    cachedDsos: null
};
...
function initPerformanceProfile() {
    ...
    const uaMobile = /Mobi|Android|Tablet|iPad|iPhone/i.test(navigator.userAgent);
    PERF.mobile = coarse || uaMobile || navigator.maxTouchPoints > 1;
    const cores = navigator.hardwareConcurrency || 4;
    const memory = navigator.deviceMemory || 4;
    const saveData = !!navigator.connection?.saveData;
    PERF.lowEnd = PERF.mobile && (cores <= 4 || memory <= 3 || saveData || Math.min(screen.width, screen.height) < 600);

    if (PERF.mobile) {
        PERF.targetFps = PERF.lowEnd ? 24 : 30;
        PERF.frameInterval = 1000 / PERF.targetFps;
        PERF.dprCap = PERF.lowEnd ? 1 : 1.5;
        // Keep the mobile sky dense and vibrant. The expensive part is the
        // astronomical math, which is cached; drawing a few thousand small
        // batched circles is far cheaper than the old per-star calculations.
        PERF.starMagLimit = PERF.lowEnd ? 5.2 : 5.8;
        PERF.maxLabels = PERF.lowEnd ? 9 : 15;
        PERF.fullEffects = false;
        PERF.simplifiedMW = false;
        PERF.astroCacheMs = PERF.lowEnd ? 1000 : 650;
    }
    // Star lists are built after ALL_STARS is declared/loaded.
}

function rebuildPerformanceStarLists() {
    if (!Array.isArray(ALL_STARS) || !ALL_STARS.length) return;
    PERF.sortedStars = [...ALL_STARS].sort((a,b) => (a.mag ?? 99) - (b.mag ?? 99));
    PERF.starByName = new Map(ALL_STARS.filter(s => s.name).map(s => [s.name, s]));
    PERF.mobileStars = PERF.mobile
        ? PERF.sortedStars.filter(s => (s.mag ?? 99) <= PERF.starMagLimit)
        : ALL_STARS;
}

function renderStarSource() {
    return PERF.mobile ? (PERF.mobileStars || ALL_STARS) : ALL_STARS;
}

function recordFrameCost(now, cost) {
    if (!PERF.mobile) return;
    PERF.frameSamples.push(cost);
    if (PERF.frameSamples.length > 30) PERF.frameSamples.shift();
    if (PERF.adaptiveCooldown > now) return;
    if (PERF.frameSamples.length < 20) return;
    const avg = PERF.frameSamples.reduce((a,b)=>a+b,0) / PERF.frameSamples.length;
    if (avg > PERF.frameInterval * 1.35 && PERF.targetFps > 24) {
        PERF.targetFps = PERF.targetFps >= 40 ? 30 : 24;
        PERF.frameInterval = 1000 / PERF.targetFps;
        PERF.starMagLimit = Math.max(3.5, PERF.starMagLimit - 0.5);
        PERF.maxLabels = Math.max(6, PERF.maxLabels - 2);
        rebuildPerformanceStarLists();
        PERF.adaptiveCooldown = now + 4000;
    } else if (avg < 18 && PERF.targetFps < 30 && !PERF.lowEnd) {
        PERF.targetFps = 30;
        PERF.frameInterval = 1000 / PERF.targetFps;
        PERF.adaptiveCooldown = now + 5000;
    }
}

function isSkyRenderingPage() {
    return arMode || !document.body.dataset.page || document.body.dataset.page === 'sky';
}

function shouldRenderFrame(now) {
    if (PERF.paused || document.hidden) return false;
    if (!isSkyRenderingPage()) return false;
    const targetInterval = (PERF.mobile && mobilePanActive) ? (1000 / 60) : PERF.frameInterval;
    if (!PERF.lastFrame || now - PERF.lastFrame >= targetInterval - 0.5) {
        PERF.lastFrame = now;
        return true;
    }
    return false;
...
window.addEventListener('resize', () => { if (PERF.mobile) { PERF.frameSamples.length = 0; } });
window.addEventListener('orientationchange', () => setTimeout(() => { if (PERF.mobile) resizeCanvas(); }, 80));
document.addEventListener('visibilitychange', () => {
    PERF.paused = document.hidden;
    if (!document.hidden) PERF.lastFrame = 0;
});
...
function updateMapHint() {
    ...
    if (arMode) {
        ...
        hint.textContent = rotateMode ? 'drag to rotate · pinch to zoom · tap for info · long-press to explore' : 'drag to pan · pinch to zoom · tap for info · long-press to explore';
    } else if (rotateMode) {
        hint.textContent = 'drag to rotate · scroll/pinch to zoom · hover for info · hold an object to explore';
    } else {
...
function constrainPan() {
    ...
    const w = canvas.width / Math.min(window.devicePixelRatio || 1, PERF.dprCap);
    const h = canvas.height / Math.min(window.devicePixelRatio || 1, PERF.dprCap);
...
function turnCameraOn() {
    ...
    if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
        ...
        .then(stream => {
            ...
            // Keep the astronomy canvas genuinely transparent over the live feed.
            canvas.style.background = 'transparent';
            canvas.style.backgroundColor = 'transparent';
...
const journeyCtx = journeyCanvas ? journeyCanvas.getContext('2d') : null;
...
function resizeJourneyCanvas() {
    ...
    cosmicJourneyDpr = Math.min(window.devicePixelRatio || 1, 2);
...
function drawJourneyFrame(now) {
    ...
    const haze = journeyCtx.createRadialGradient(cx, cy, 0, cx, cy, Math.max(w,h)*.7);
    haze.addColorStop(0, `rgba(40,70,100,${0.055 * (1-p)})`);
    haze.addColorStop(.5, `rgba(20,30,55,${0.035 * (1-p)})`);
    haze.addColorStop(1, 'rgba(0,0,0,0)');
    journeyCtx.fillStyle = haze;
    journeyCtx.fillRect(0,0,w,h);
    ...
    cosmicJourneyStars.forEach(st => {
        ...
        if (p > .18 && p < .82 && st.radius > 35) {
            ...
            journeyCtx.beginPath(); journeyCtx.moveTo(sx - ux * streak, sy - uy * streak); journeyCtx.lineTo(sx, sy); journeyCtx.stroke();
        }
    });
...
function drawMilkyWayFilaments(ctx, astroTime, observer, mwFader, pixelPerDeg) {
    ...
    MW_FILAMENTS.forEach((filament, fi) => {
        ...
        filament.forEach((pt, qi) => {
            const pre = precessStarToDate(pt.raHrs * 15, pt.dec, astroTime);
            const hor = Astronomy.Horizon(astroTime, observer, pre.ra, pre.dec, dynamicRefraction);
...
function drawPhotorealMilkyWay(ctx,astroTime,observer,mwFader,r) {
    if (PERF.simplifiedMW) {
        const w = canvas.width / Math.min(window.devicePixelRatio || 1, PERF.dprCap);
        const h = canvas.height / Math.min(window.devicePixelRatio || 1, PERF.dprCap);
        const pixelPerDeg = r / 90;
        ctx.save();
        ctx.globalCompositeOperation = 'screen';
        // Reuse the prebuilt soft brush instead of creating hundreds of radial gradients.
        for (let i = 0; i < MW_BLOBS.length; i += 6) {
            ...
            const rr = blob.sizeDeg * pixelPerDeg * 1.35;
            ctx.globalAlpha = Math.min(0.10, blob.alpha * 2.2 * mwFader);
            ctx.drawImage(mwBrush, p.x - rr, p.y - rr, rr * 2, rr * 2);
        }
        ctx.globalAlpha = 1;
        ctx.restore();
        return;
...
function loadExpandedStarCatalog() {
    ...
    PERF.sortedStars = null; PERF.mobileStars = null;
    rebuildPerformanceStarLists();
...
function renderOptimizedStars(ctx, astroTime, observer, starDimFactor, w, h) {
    ...
    const starsToRender = renderStarSource();
    const starLocationKey = `${lat.toFixed(4)}:${lon.toFixed(4)}`;
    const useCachedStarMath = PERF.mobile && Number.isFinite(PERF.starCacheStamp) && (performance.now() - PERF.starCacheStamp < PERF.astroCacheMs) && PERF.starCacheLocationKey === starLocationKey;
    if (PERF.mobile && !useCachedStarMath) { PERF.starCacheStamp = performance.now(); PERF.starCacheLocationKey = starLocationKey; }

    for (let i = 0; i < starsToRender.length; i++) {
        ...
        let alt, az;
        if (useCachedStarMath && Number.isFinite(star._cachedAlt)) {
            alt = star._cachedAlt;
            az = star._cachedAz;
        } else {
            ...
            if (PERF.mobile) { star._cachedAlt = alt; star._cachedAz = az; }
        }
    ...
    if (photorealisticMode && !arMode && PERF.fullEffects) {
        ctx.save();
        ctx.globalCompositeOperation = 'screen';
    ...
    for (let k = 0; k < brightStarsToGlow.length; k++) {
        ...
        if (photorealisticMode && PERF.fullEffects) {
...
function drawMap(now = performance.now()) {
    if (PERF.paused || document.hidden || !isSkyRenderingPage()) return;
    ...
    try {
    // During finger interaction/inertia, keep the star/planet rendering at full
    // quality but avoid paying for the photographic Milky Way layer.
    if (PERF.mobile && mobilePanActive && !arMode && !isInteracting && mobilePanInertia) {
        panX += panVelocityX;
        panY += panVelocityY;
        panVelocityX *= mobilePanFriction;
        panVelocityY *= mobilePanFriction;
        constrainPan();
        if (Math.abs(panVelocityX) < mobilePanStopThreshold && Math.abs(panVelocityY) < mobilePanStopThreshold) {
            panVelocityX = panVelocityY = 0;
            mobilePanInertia = false;
            mobilePanActive = false;
        }
    ...
    if (mwFader > 0.01 && !(PERF.mobile && mobilePanActive && !arMode)) {
        ctx.save();
        ctx.globalCompositeOperation = 'screen';
        ...
        MW_BLOBS.forEach(blob => {
            ...
            const blobRadius=blob.sizeDeg*pixelPerDeg*(arMode?1:visualZoomLevel);
        ...
        MW_STARS.forEach(star=>{
            ...
            const starRadius=star.sizeDeg*(arMode?1:visualZoomLevel); ctx.beginPath();ctx.arc(proj.x,proj.y,starRadius,0,Math.PI*2);ctx.fill();
        }); ctx.restore();
    }

    let planetPositions = [];
    const objectLocationKey = `${lat.toFixed(4)}:${lon.toFixed(4)}`;
    const useObjectCache = PERF.mobile && PERF.cachedPlanets && PERF.cachedDsos && PERF.objectCacheLocationKey === objectLocationKey && (performance.now() - PERF.objectCacheStamp < PERF.astroCacheMs);
    ...
    let dsoPositions = [];
    if (useObjectCache) {
        planetPositions = PERF.cachedPlanets.map(p => ({...p, ...projectAz(p.alt, p.az)}));
        dsoPositions = PERF.cachedDsos.map(d => ({...d, ...projectAz(d.alt, d.az)}));
    } else {
    ...
    dsoPositions = DSO_OBJECTS.map(obj => {
        ...
        PERF.cachedPlanets = planetPositions.map(p => { const q={...p}; delete q.x; delete q.y; delete q.z3d; return q; });
        PERF.cachedDsos = dsoPositions.map(d => { const q={...d}; delete q.x; delete q.y; delete q.z3d; return q; });
        PERF.objectCacheStamp = performance.now();
        PERF.objectCacheLocationKey = objectLocationKey;
    }
    ...
    if (showConstellations) {
        ...
        CONSTELLATION_LINES.forEach(([n1, n2]) => {
            const s1 = PERF.starByName.get(n1);
            const s2 = PERF.starByName.get(n2);
            if (!s1 || !s2) return;
            if (
    ...
    if (starDimFactor > 0.1) {
        ...
        const starLimit = arMode ? Math.min(PERF.maxLabels, PERF.mobile ? PERF.maxLabels : 60) : (PERF.mobile ? Math.min(10, PERF.maxLabels) : 12);
        const labelSource = PERF.sortedStars || ALL_STARS;
        const labeledStars = [];
        for (let li = 0; li < labelSource.length && labeledStars.length < starLimit; li++) {
            const star = labelSource[li];
            if (star.onScreen && star.alt >= 0.0 && star.name) labeledStars.push(star);
        }
    ...
    if (actualSunHor.altitude > 0 && currentSunAnim > 0.5 && !sunForcedOff && !arMode) {
        ...
        ctx.fillRect(0, 0, canvas.width / Math.min(window.devicePixelRatio || 1, PERF.dprCap), canvas.height / Math.min(window.devicePixelRatio || 1, PERF.dprCap));
        ctx.fillStyle = '#ff4b4b';
        ctx.font = '500 14px "Space Grotesk", sans-serif';
        ctx.textAlign = 'center';
        ctx.fillText('Star map unavailable', canvas.width / Math.min(window.devicePixelRatio || 1, PERF.dprCap) / 2, canvas.height / Math.min(window.devicePixelRatio || 1, PERF.dprCap) / 2);
    }
    ...
    recordFrameCost(now, performance.now() - paintStart);
    requestAnimationFrame(drawMap);
}
...
function checkTooltip(clientX, clientY) {
    ...
    if (closest) {
        ...
        info += ' · ' + visCat.label;
        info += ' · Press and hold for Cosmic Journey';
...
canvas.addEventListener('touchstart', e => {
    ...
    if (!arMode && PERF.mobile && e.touches.length === 1) {
    ...
    if (e.touches.length === 1 && isInteracting) {
        ...
        if (arMode) {
            ...
            if (PERF.mobile) {
                ...
                const dtPan = Math.max(8, nowPan - lastPanMoveTime);
                panVelocityX = (dx / dtPan) * 16.67;
                panVelocityY = (dy / dtPan) * 16.67;
                lastPanMoveTime = nowPan;
                mobilePanActive = true;
                mobilePanInertia = false;
                PERF.lastFrame = 0;
            }
...
canvas.addEventListener('touchend', e => {
    ...
    if (e.touches.length === 0) {
        ...
        touchStartDist = 0;
        constrainPan();
        if (!arMode && PERF.mobile) {
            const speed = Math.hypot(panVelocityX, panVelocityY);
            if (speed > 0.25 && touchMoved) {
                mobilePanActive = true;
                mobilePanInertia = true;
                PERF.lastFrame = 0;
            } else {
                mobilePanActive = false;
                mobilePanInertia = false;
        ...
        if (!arMode && PERF.mobile) { mobilePanActive = true; mobilePanInertia = false; panVelocityX = panVelocityY = 0; PERF.lastFrame = 0; }
        isInteracting = true;
        interactStartX = e.touches[0].clientX; interactStartY = e.touches[0].clientY;
        if (!arMode) {
...
function applyTimeChange() {
    PERF.starCacheStamp = 0; PERF.objectCacheStamp = 0;
    refreshTimeTravelUI();
    updatePlanets();
    drawMoon();
}
...
function startTimelapse() {
    ...
    updateTimelapseButtons();
    timelapseLastFrame = performance.now();
    timelapseUiAccumulator = 0;

    const tick = (now) => {
        if (!timelapseActive) return;
        const dt = Math.min(100, Math.max(0, now - timelapseLastFrame));
        timelapseLastFrame = now;

        // Preserve the original 1.5 simulated minutes / 30 ms speed, but advance
        // the simulation from the existing render loop instead of forcing a second
        // 33 Hz render/update loop on top of drawMap().
        timeOffsetMinutes += dt * (TIMELAPSE_STEP_MINUTES / TIMELAPSE_TICK_MS);
        timelapseUiAccumulator += dt;

        // Expensive planet/moon calculations and DOM work are throttled while the
        // sky canvas itself remains visually smooth at the browser's native RAF rate.
        if (timelapseUiAccumulator >= 180) {
            timelapseUiAccumulator = 0;
            refreshTimeTravelUI();
            updatePlanets();
            drawMoon();
        }
        timelapseRafId = requestAnimationFrame(tick);
    };
...
function stopTimelapse() {
    ...
    if (timelapseRafId) cancelAnimationFrame(timelapseRafId);
    timelapseRafId = null;
