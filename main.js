...
function exitAR() {
    ...
    if (arMode) {
        arMode = false;
        document.body.classList.remove('ar-active'); // Add this line
        stopARCameraAndSensors();
        restoreMapUI();
    }
...
function drawMap() {
        ...
        if (!cameraActive) {
            ...
            FAINT_STARS_AR.forEach(fs => {
                ...
                if (p.onScreen && p.z3d > 0.01) {
                    ...
                    ctx.beginPath(); ctx.arc(p.x, p.y, 0.7 * zoomLevel, 0, Math.PI * 2); ctx.fill();
                }
            });
            ctx.globalAlpha = 1.0;
        }
    ...
    if (starDimFactor > 0.1) {
        const limit = arMode ? 80 : 15;
        const labeledStars = starPositions.filter(s => s.onScreen && s.alt >= 0.0).sort((a, b) => a.mag - b.mag).slice(0, limit);
        
        labeledStars.forEach((star) => {
            const horizonFade = star.alt < 10 ? Math.max(0, star.alt / 10) : 1;
            const fontStr = arMode ? '500 15px "Space Grotesk", sans-serif' : '500 9px "Space Grotesk", sans-serif';
            drawSmartLabel(ctx, star.name, star.x, star.y, `rgba(255,255,255,${arMode ? 1.0 : 0.4 * horizonFade * starDimFactor})`, fontStr, drawnLabels, false); 
        });
        ...
        labeledDso.forEach(d => {
            ...
            const fontStr = arMode ? 'italic 500 14px "Space Grotesk", sans-serif' : 'italic 500 8px "Space Grotesk", sans-serif';
            drawSmartLabel(ctx, d.name, d.x, d.y, `rgba(201,169,110,${arMode ? 0.9 : 0.35 * horizonFade * starDimFactor})`, fontStr, drawnLabels, false);
        });
    }
