...
// --- IDLE RENDER-RATE GOVERNOR ---
// drawMap() re-runs its whole pipeline (sky gradient, atmosphere, the ~5,000-star
// catalogue's projection + paint passes, Moon/planets/DSOs, horizon, labels) on
// every single requestAnimationFrame callback, uncapped. On a 60Hz phone that's
// 60 full redraws/sec; on a 90-120Hz phone (common on current mid-range hardware)
// it's up to double that — for a view that is visually static almost all of the
// time, since real sky motion is only ~15 arcsec/sec. Sustained full-refresh-rate
// redraws are exactly what drives thermal throttling on longer sessions. While
// the user isn't actively dragging/pinching/timelapsing/journeying, we throttle
// to IDLE_TARGET_FPS; the instant a gesture starts, rendering goes back to the
// display's native rate so panning/zooming/Timelapse/Voyager stay exactly as
// smooth as before. No visual output changes — only how many times per second an
// unchanging (or slowly changing) frame gets redrawn.
const IDLE_TARGET_FPS = 30;
const IDLE_FRAME_INTERVAL_MS = 1000 / IDLE_TARGET_FPS;
...
function drawMap() {
    ...
    try {
        ...
        if (!isActivelyGesturing()) {
            ...
            if (lastDrawMapRenderMs >= 0 && (gateNowMs - lastDrawMapRenderMs) < IDLE_FRAME_INTERVAL_MS) {
    ...
    if (timelapseActive) {
        ...
        // Ancillary panels are intentionally refreshed much less often during
        // Timelapse so their DOM/canvas work cannot periodically hitch the main
        // cinematic sky render. Their next normal refresh is restored on stop.
        timelapseUiAccumulator = elapsedMs;
        if (elapsedMs - (timelapseLastFrame || timelapseStartWallMs) >= 1000) {
    ...
    if (mwFader > 0.01) {
        ...
        MW_BLOBS.forEach(blob => {
            ...
            const proj=arMode ? projectCachedSkyPoint(blob) : projectAz(hor.altitude,hor.azimuth); if(!proj.onScreen)return;
        ...
        if (arMode) {
            ...
            // Only active gestures use the lightweight MW path. Once the finger
            // lifts, the full photographic renderer returns on the next frame.
            drawLightweightMilkyWay(ctx, astroTime, observer, mwFader, r, w, h);
        } else {
            // Mobile idle mode deliberately keeps the gorgeous full treatment.
            if (photorealisticMode) drawPhotorealMilkyWay(ctx,astroTime,observer,mwFader,r);
            ...
            MW_STARS.forEach(star=>{
                ...
                const starRadius=star.sizeDeg*visualZoomLevel; ctx.beginPath();ctx.arc(proj.x,proj.y,starRadius,0,Math.PI*2);ctx.fill();
            }); ctx.restore();
        }
    }
    ...
    const dsoPositions = DSO_OBJECTS.map(obj => {
        const hor = getCachedHorizon(obj, obj.ra, obj.dec, astroTime, observer);
        const proj = arMode ? projectCachedSkyPoint(obj) : projectAz(hor.altitude, hor.azimuth);
