...
function freezeARTrackingForManualGesture() {
    ...
    // Preserve the live orientation before switching to manual touch control.
    // The previous implementation reset the smoothed orientation too early,
    // which could snap the AR sky to an arbitrary azimuth/altitude on the first drag.
    if (Number.isFinite(smoothAlpha)) syntheticAzimuth = (360 - smoothAlpha + 360) % 360;
...
window.addEventListener('mousemove', e => {
    ...
    if (arMode) {
        if (cameraActive) return; // Prevent panning AR sky away from live camera feed
...
canvas.addEventListener('touchmove', e => {
    ...
    if (e.touches.length === 1 && isInteracting) {
        ...
        if (Math.abs(e.touches[0].clientX - interactStartX) > 7 || Math.abs(e.touches[0].clientY - interactStartY) > 7) {
            ...
            if (arMode && arTrackingActive && !cameraActive) {
                freezeARTrackingForManualGesture();
