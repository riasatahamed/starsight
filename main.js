...
// Performance cache: star astronomy math changes extremely slowly compared with
// the visual frame rate. Keep the exact same rendering, but reuse calculated
// alt/az + screen positions for a short interval on non-AR mobile frames.
// This removes thousands of trig/Astronomy operations from most frames without
// changing the catalogue, sizes, colours, labels, or visual design.
let starProjectionCache = null;
let starProjectionCacheKey = '';
let starProjectionCacheAt = 0;
const STAR_PROJECTION_CACHE_MS = 250;

function renderOptimizedStars(ctx, astroTime, observer, starDimFactor, w, h) {
    const nowMs = performance.now();
    const astronomyCacheKey = [
        Math.round(astroTime.tt * 86400),
        observer.latitude.toFixed(4), observer.longitude.toFixed(4)
    ].join('|');
    const cacheKey = [
        astronomyCacheKey, zoomLevel.toFixed(4), panX.toFixed(1), panY.toFixed(1), rotateOffset.toFixed(3),
        arMode ? 1 : 0, arMode ? R_matrix.join(',') : '',
        screenOrientation
    ].join('|');

    const canReuse = !arMode && starProjectionCache &&
        starProjectionCacheKey === cacheKey &&
        (nowMs - starProjectionCacheAt) < STAR_PROJECTION_CACHE_MS;
    ...
    if (canReuse) {
        for (const key in FAINT_COLOR_BUCKETS) {
            FAINT_COLOR_BUCKETS[key].stars = starProjectionCache.buckets[key];
        }
        brightStarsToGlow.push(...starProjectionCache.bright);
    } else {
        const buckets = {};
        for (let key in FAINT_COLOR_BUCKETS) {
            FAINT_COLOR_BUCKETS[key].stars.length = 0;
            buckets[key] = [];
        }
    ...
    for (let i = 0; i < ALL_STARS.length; i++) {
        ...
        let alt, az;
        const positionCacheValid = star._positionCacheKey === astronomyCacheKey;
        ...
        // Cache only the expensive celestial-position calculation. Screen projection
        // still runs every frame, so AR orientation, drag, zoom and rotation remain
        // fully responsive.
        star._positionCacheKey = astronomyCacheKey;
        star._cachedAlt = alt;
        star._cachedAz = az;
        ...
        // Store the exact same star objects/positions for the next few frames.
        // Copy the bucket arrays so subsequent rendering cannot mutate the cache.
        for (const key in FAINT_COLOR_BUCKETS) {
            buckets[key] = FAINT_COLOR_BUCKETS[key].stars.slice();
        }
        starProjectionCache = { buckets, bright: brightStarsToGlow.slice() };
        starProjectionCacheKey = cacheKey;
        starProjectionCacheAt = nowMs;
    }
...
// Mobile performance governor. The visual scene is unchanged; we simply avoid
// rendering duplicate frames faster than the display can meaningfully show them.
const mobilePerformanceMode = window.matchMedia('(max-width: 1024px), (pointer: coarse)').matches;
const targetFrameMs = mobilePerformanceMode ? (1000 / 45) : 0;
let lastMapRenderAt = 0;

function drawMap() {
    const frameNow = performance.now();
    if (targetFrameMs && frameNow - lastMapRenderAt < targetFrameMs) {
        requestAnimationFrame(drawMap);
        return;
    }
    lastMapRenderAt = frameNow;
    try {
    const w = canvas.width / Math.min(window.devicePixelRatio, 2);
    const h = canvas.height / Math.min(window.devicePixelRatio, 2);
    const cx = w/2 + panX;
