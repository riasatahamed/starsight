...
async function handlePlateSolve(file) {
    ...
    // Check for API key in local storage, prompt user if missing
    ...
    try {
        ...
        if (loginData.status !== "success") {
            // If the key is invalid, remove it so the prompt appears next time
        ...
        console.error(e);
        alert("Plate solving error: Check console or ensure your API key is correct.");
...
async function pollPlateSolveStatus(subid) {
    const interval = setInterval(async () => {
        try {
            const statRes = await fetch(`https://nova.astrometry.net/api/submissions/${subid}`);
            ...
            if (statData.jobs && statData.jobs.length > 0) {
                ...
                if (jobData.status === 'success') {
                    ...
                    const calRes = await fetch(`https://nova.astrometry.net/api/jobs/${jobId}/calibration`);
